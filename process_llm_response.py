...
def postprocess_llm_response():
  ...
  # for pid in range(1, 2059):
  #   config = {"configurable": {"thread_id": str(pid)}}
  #   data = checkpointer.get(config)
  #   response_text = data["channel_values"]["messages"][-1].content
  #   json_data = {"persona_id": f"pid_{pid}", "response_text": response_text}

  #   persona_file_path = simulation_output_dir / f"pid_{pid}"
  #   persona_file_path.mkdir(parents=True, exist_ok=True)
    
  #   with open(persona_file_path / f"pid_{pid}_response.json", "w") as f:
